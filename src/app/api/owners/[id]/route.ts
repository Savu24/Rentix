...
export async function GET(_request: NextRequest, { params }: Params) {
  ...
  if (!owner) return apiError("NOT_FOUND", auth.d.panel.api.notFound.owner);
  return ok(owner);
}
...
export async function PATCH(request: NextRequest, { params }: Params) {
  ...
  const parsed = ownerUpdateSchema(auth.v).safeParse(body);
  ...
  if ((body as { restore?: unknown }).restore === true) {
    ...
    if (!restored) return apiError("NOT_FOUND", auth.d.panel.api.notFound.archivedOwner);
...
export async function DELETE(request: NextRequest, { params }: Params) {
  ...
  if (!force) {
    ...
    if (!archived) return apiError("NOT_FOUND", auth.d.panel.api.notFound.owner);
  ...
  if (result.reason === "NOT_FOUND") return apiError("NOT_FOUND", auth.d.panel.api.notFound.owner);
