# Resend krok po kroku — po zakupie rentixon.com
...
Zakładam domenę **rentixon.com** i wysyłkę z subdomeny
**powiadomienia.rentixon.com**.
...
`rentixon.com` pójdzie newsletter, mailing do inwestorów albo poczta firmowa
...
- **domena w Resendzie**: `powiadomienia.rentixon.com` (subdomena)
- **adres nadawcy**: `powiadomienia@rentixon.com` (skrzynka na domenie głównej)

To dwie różne rzeczy i muszą być spójne. Jeśli w Resendzie dodasz subdomenę
`powiadomienia.rentixon.com`, to adres nadawcy musi być **na tej subdomenie**,
czyli np. `czynsz@powiadomienia.rentixon.com`. Masz dwie drogi:

| Droga | Domena w Resendzie | `EMAIL_FROM` |
|---|---|---|
| A — prościej | `rentixon.com` | `Rentix <powiadomienia@rentixon.com>` |
| B — czyściej | `powiadomienia.rentixon.com` | `Rentix <czynsz@powiadomienia.rentixon.com>` |

**Wybierz A, jeśli `rentixon.com` nie będzie służyć do niczego innego** — a na
...
Wchodzisz do panelu DNS tam, gdzie kupiłeś domenę (Cloudflare, Porkbun, OVH).
...
a panel sam zrobi z tego `resend._domainkey.rentixon.com`. Jeśli wpiszesz pełną
nazwę ręcznie, wyjdzie `resend._domainkey.rentixon.com.rentixon.com`
i weryfikacja nigdy nie przejdzie — a komunikat błędu tego nie powie.

Po wklejeniu sprawdź sobie z konsoli, czy rekordy widać:

```
nslookup -type=TXT resend._domainkey.rentixon.com
nslookup -type=TXT send.rentixon.com
nslookup -type=MX send.rentixon.com
```
...
W panelu Resendu: **Domains → rentixon.com → Verify**. Zielony status przy
...
| Typ | Nazwa | Wartość |
|---|---|---|
| TXT | `_dmarc` | `v=DMARC1; p=none; rua=mailto:dmarc@rentixon.com` |
...
Skrzynka `dmarc@rentixon.com` musi istnieć albo mieć przekierowanie —
...
4. Domena: ogranicz do `rentixon.com`.
...
| Resend | domena `rentixon.com`, klucz API z Sending access |
